        ...
        const participantsList = details.participants.length > 0 
          ? `<ul class="participants-list">${details.participants.map(participant => 
              `<li>
                ...
                <button class="delete-button" onclick="unregisterParticipant('${name}', '${participant}')" title="Remove participant">
                  ✗
                </button>
              </li>`
            ).join('')}</ul>`
  ...
  window.unregisterParticipant = async function(activityName, email) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log("Refreshing activities after successful unregistration...");
  ...
  signupForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log("Refreshing activities after successful signup...");
        await fetchActivities();
      } else {
        messageDiv.textContent = result.detail || "An error occurred";
        messageDiv.className = "error";
